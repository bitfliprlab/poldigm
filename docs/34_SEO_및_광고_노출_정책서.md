...
| 전역 오류 화면 | `noindex,nofollow` | 잘못된 라우트와 예외 화면 |
...
<meta name="description" content="20개의 딜레마 문항으로 나의 사회적·정치적 가치관과 16가지 스펙트럼을 확인해보세요.">
...
- 로컬에서는 `PUBLIC_APP_BASE_URL=http://127.0.0.1:5173`을 사용하되, 배포 전 실제 도메인으로 교체한다.
- `/test`, `/analysis`, `/result/[id]`, 전역 오류 화면은 페이지별 robots 값을 반드시 덮어쓴다.

### 4.2. 페이지별 title/description

| 라우트 | title | description |
|---|---|---|
| `/` | `Poldigm - 나의 사회적·정치적 가치관 테스트` | `20개의 딜레마 문항으로 나의 사회적·정치적 가치관과 16가지 스펙트럼을 확인해보세요.` |
| `/test` | `테스트 진행 중 - Poldigm` | `Poldigm 테스트를 진행 중입니다.` |
| `/analysis` | `결과 분석 중 - Poldigm` | `응답을 분석해 나의 사회적·정치적 가치관을 계산하고 있습니다.` |
| `/result/[id]` | `{title} - Poldigm 결과` | `{subtitle} Poldigm에서 나의 사회적·정치적 가치관을 확인해보세요.` |
| `/terms` | `이용약관 - Poldigm` | `Poldigm 서비스 이용약관입니다.` |
| `/privacy` | `개인정보처리방침 - Poldigm` | `Poldigm 개인정보처리방침입니다.` |
| 전역 오류 화면 | `페이지를 찾을 수 없어요 - Poldigm` | `요청한 Poldigm 페이지를 찾을 수 없습니다. 홈에서 테스트를 다시 시작할 수 있습니다.` |
...
<meta property="og:title" content="Poldigm - 나의 사회적·정치적 가치관 테스트">
<meta property="og:description" content="20개의 딜레마 문항으로 나의 사회적·정치적 가치관과 16가지 스펙트럼을 확인해보세요.">
...
<meta name="twitter:title" content="Poldigm - 나의 사회적·정치적 가치관 테스트">
<meta name="twitter:description" content="20개의 딜레마 문항으로 나의 사회적·정치적 가치관과 16가지 스펙트럼을 확인해보세요.">
...
- `og:description`: `{subtitle} 20개 딜레마로 나의 사회적·정치적 가치관을 확인해보세요.`
- `og:url`: 현재 결과 URL
...
로컬 MVP와 실제 MVP 모두 SvelteKit endpoint로 `robots.txt`를 제공합니다.
...
현재 구현 파일은 `src/routes/robots.txt/+server.ts`입니다. `robots.txt`는 sitemap 위치를 알리는 최소 파일로 둡니다. `/test`, `/analysis`, `/result/[id]`, 전역 오류 화면 같은 비색인 페이지는 robots 차단이 아니라 페이지별 `meta name="robots"`의 `noindex`로 처리합니다. robots에서 차단하면 검색엔진이 페이지의 `noindex`를 읽지 못할 수 있으므로, 비색인 정책은 페이지 메타를 기준으로 합니다.

### 6.2. `sitemap.xml`

현재 sitemap에는 정적 공개 페이지만 포함합니다.
...
현재 구현 파일은 `src/routes/sitemap.xml/+server.ts`입니다. URL은 `PUBLIC_APP_BASE_URL`을 기준으로 생성합니다.
...
Google AdSense 적용은 가능하다고 보되, Poldigm의 주제가 사회적·정치적 가치관으로 해석될 수 있으므로 **중간 리스크**로 분류합니다.
...
- 서비스 포지셔닝은 “정치 성향 판정”보다 “사회적·정치적 가치관 테스트”로 유지한다.
...
Poldigm은 사회적·정치적 가치관을 다루므로 광고 플랫폼 정책상 민감 주제, 개인화 광고, 정치적 콘텐츠 해석 리스크가 있을 수 있습니다.
...
- `/test`, `/analysis`, 전역 오류 화면에 `noindex,nofollow` 적용
...
| 비색인 페이지 | `/test`, `/analysis`, `/result/[id]`, 전역 오류 화면의 `meta robots` 값 확인 |
...
- `/`, `/terms`, `/privacy`는 `index,follow`
- `/`, `/terms`, `/privacy`는 `PUBLIC_APP_BASE_URL` 기준 canonical 제공
- `/test`는 `noindex,nofollow`
- `/result/[id]`는 `noindex,follow`
- 전역 오류 화면은 `noindex,nofollow`
- `robots.txt`, `sitemap.xml` 응답
...
로컬 MVP는 광고, Search Console, 실제 OG 이미지 배포를 구현 범위에 포함하지 않습니다. `robots.txt`와 `sitemap.xml`은 로컬에서도 검증 가능한 endpoint로 구현했습니다.
...
| robots/sitemap | `src/routes/robots.txt/+server.ts`, `src/routes/sitemap.xml/+server.ts` 구현 |
| AdSense | 미삽입 |
| Analytics | 미삽입 |
| 결과 페이지 색인 | `noindex,follow` 적용 |
