...
```json
{
  ...
  "question": {
    ...
    "choices": {
      "A": "예외가 줄어들도록 같은 규칙을 먼저 적용한다.",
      "B": "조금 복잡해져도 각자의 일하는 방식을 더 인정한다."
    },
    "display": {
      "promptLines": [
        "공유 사무실에서 모두 같은 예약 규칙을",
        "쓰자는 말이 나옵니다."
      ],
      "promptHighlights": ["규칙"],
      "choices": {
        "A": {
          "label": "먼저 막기",
          "body": "예외가 줄어들도록 같은 규칙을 먼저 적용한다.",
          "highlights": ["먼저", "규칙"]
        },
        "B": {
          "label": "동의 먼저",
          "body": "조금 복잡해져도 각자의 일하는 방식을 더 인정한다.",
          "highlights": ["동의", "먼저", "각자"]
        }
      }
    },
...
- 선택지 라벨은 축 설명이 아니라 `먼저 막기`, `동의 먼저`, `그대로 간다`, `한발 물린다`처럼 짧은 행동 기준으로 작성합니다.
- 문항 카피는 정책 찬반 문장보다 아파트 단톡방, 공용 공간, 학교 행사, 직장 보상, 공원 이용처럼 바로 이해되는 생활 장면형 딜레마를 우선합니다.
...
```json
{
  ...
  "branchCondition": "C",
  "branchOrder": 1,
  "prompt": "당신이 세운 확인 절차 때문에 한 사람이 계속 감시받는다고 느낍니다.",
  "choices": {
...
```json
{
  ...
  "branchOrder": 1,
  "prompt": "분실 사고가 잦은 공간에서 출입 기록을 남길지 정해야 합니다.",
  "choices": {
    "A": "기록을 남기되 열람 권한과 보관 기간을 짧게 묶는다.",
    "B": "기록 공유는 동의한 사람에게만 좁게 적용한다."
  },
  "scoreEffect": {
    "A": { "C": 20 },
    "B": { "I": 20 }
