...
본 문서는 폴다임(Poldigm)을 **외부 인프라 없이 로컬 개발 환경에서 실행 가능한 MVP**로 구현하기 위한 범위, 방식, 파일 구조, 구현 순서, 완료 기준을 정의합니다.

이 문서는 실제 상용 배포용 아키텍처 문서인 `30_시스템_아키텍처_설계서`, API 계약 문서인 `31_API_데이터_스키마_명세서`, DB 문서인 `32_DB스키마_및_RLS정책서`보다 **로컬 개발 실행성**을 우선합니다. 즉, 로컬 MVP는 Supabase, Cloudflare Pages, Turnstile, 실제 도메인 연결 없이도 `localhost`에서 테스트 전체 플로우가 동작해야 합니다.
...
| Turnstile | 실제 검증 없이 local mock token으로 통과 처리 |
...
| Cloudflare Pages 배포 | 고려하지 않음. `localhost` 실행만 목표 |
| Cloudflare Turnstile 실제 검증 | 하지 않음. 로컬에서는 검증 성공으로 mock 처리 |
| 실제 도메인 연결 | 하지 않음 |
...
| Language | TypeScript |
| Styling | CSS 변수 + Svelte scoped CSS |
| Adapter | 로컬 MVP에서는 기본 dev server 사용. 배포 adapter 설정은 후속 단계 |
...
서버 코드는 `PUBLIC_APP_ENV === 'local'` 또는 별도 local adapter 설정을 기준으로 mock 저장/검증을 사용합니다.
...
### 6.1. Placeholder 문항 정책

로컬 MVP의 우선순위는 문항 카피 완성도가 아니라 **분기/점수/결과 산출 플로우 검증**입니다.

초기 구현에서는 placeholder 문구를 허용했지만, 현재 로컬 MVP는 44개 문항 슬롯을 정상 한국어 카피로 채웠습니다. 향후 문구를 교체하더라도 각 문항은 아래 구조를 반드시 만족해야 합니다.
...
구현 완료 판단은 “44개 최종 카피 완성”이 아니라 “44개 슬롯의 구조 검증과 20문항 플로우 정상 동작”을 기준으로 합니다.

QA 완료 기준은 아래처럼 분리합니다.

- 알고리즘 QA: placeholder 문항이어도 44개 슬롯 구조, 분기 도달성, 점수 계산, S/M 판별이 정상 동작하면 통과
- 콘텐츠 QA: 최종 문항 카피로 교체한 뒤 표현 수위, 문항 편향, 오탈자, 선택지 균형을 별도로 검수해야 통과
...
- `sessionStorage` 데이터의 `resultId`가 URL의 `id`와 같으면 해당 데이터로 결과 화면을 렌더링한다.
...
- 클라이언트는 `turnstileToken: 'local-mock-token'`을 전송
- 서버는 local 환경에서 해당 값을 검증 성공으로 처리
...
- `package.json`, `src/`, `static/` 생성
- TypeScript 사용
- 전역 CSS와 기본 라우팅 설정

### Step 2. 디자인 토큰 적용

- `src/app.css`에 `21_디자인_시스템_가이드`의 CSS 변수 반영
- 모바일 최대 폭, 다크 테마, 기본 폰트 설정

### Step 3. 서버 데이터 작성

- `questions.ko-KR.ts`에 MVP 필수 44문항 슬롯 구성
...
- local 환경에서 `turnstileToken: 'local-mock-token'`은 통과한다.
...
SvelteKit 스캐폴딩과 첫 구현이 끝나면 본 문서의 예시 구조를 실제 코드와 대조합니다.

확인 항목:

- `package.json`의 스크립트가 `npm run dev`, `npm run test`, 필요 시 `npm run check`와 일치하는가
- 브라우저 검증 스크립트가 있다면 `npm run test:e2e`, `npm run test:e2e:chrome`과 실제 검증 범위가 문서에 반영되어 있는가
- 실제 폴더 구조가 6장의 권장 파일 구조와 크게 어긋나지 않는가
- 테스트 러너가 Vitest가 아니라면 본 문서의 Test 항목을 갱신했는가
...
- `npm run test:e2e`가 데스크톱/모바일 랜딩, 진행 중 새로고침 복구, 20문항 완료, 결과 새로고침 복구, sessionStorage fallback, 결과 없음 CTA를 검증한다.
...
- Turnstile 없이 local mock token으로 결과 제출이 가능하다.
...
| `PUBLIC_APP_BASE_URL=http://localhost:5173` | 실제 도메인 |
...
2026-06-21 기준 본 문서의 로컬 MVP 범위는 구현 및 검증 완료 상태입니다.

| 항목 | 결과 |
|---|---|
| SvelteKit 스캐폴딩 | 완료 |
| 44개 문항 슬롯 | `questions.ko-KR.ts` 구현 |
